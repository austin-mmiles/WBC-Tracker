...
def fetch_stats():
    ...
    print("🔍 Fetching WBC schedule...")
    for sport_id in [23, 51, 22]:
        try:
            url = f"{BASE}/schedule?sportId={sport_id}&season={SEASON}&startDate={WBC_START}&endDate={WBC_END}&gameType=I,W,C,P,L,F,D,S"
            data = get(url)
            ...
            if finished:
                print(f"  ✅ sportId={sport_id}: found {len(games)} total games, {len(finished)} completed")
                game_pks = finished
                break
            else:
                print(f"  ⚠️  sportId={sport_id}: {len(games)} games found but none completed yet")
                if games: game_pks = [g["gamePk"] for g in games]  # keep for debugging
        except Exception as e:
            print(f"  ❌ sportId={sport_id}: {e}")

    if not game_pks:
        print("⚠️  No completed games found. Trying season stats endpoint as fallback...")
        for sport_id in [23, 51]:
            try:
                for group in ["hitting", "pitching"]:
                    url = f"{BASE}/stats?stats=season&group={group}&sportId={sport_id}&season={SEASON}&limit=2000"
                    data = get(url)
                    splits = data.get("stats", [{}])[0].get("splits", [])
                    for s in splits:
                        merge(cache, s["player"]["fullName"], s["player"]["id"], group, s["stat"])
                if cache:
                    print(f"  ✅ sportId={sport_id} season stats: {len(cache)} players found")
                    break
            except Exception as e:
                print(f"  ❌ season stats sportId={sport_id}: {e}")
        return cache, 0

    print(f"\n📊 Fetching box scores for {len(game_pks)} games...")
    ...
    print(f"\n✅ Total players with stats: {len(cache)}")
...
def score_roster(owner, cache):
    ...
    for p in players:
        ...
        if h:
            stat_str = f"{h.get('hits',0)}H {h.get('homeRuns',0)}HR {h.get('rbi',0)}RBI {h.get('runs',0)}R"
        elif pit:
            ip = pit.get("inningsPitched","0")
            stat_str = f"{ip}IP {pit.get('strikeOuts',0)}K {pit.get('earnedRuns',0)}ER {pit.get('wins',0)}W"
        else:
            stat_str = "—"

        results.append({
            "name": p["name"], "pos": p["pos"], "nation": p["nation"],
            "pts": round(pts, 1), "h_pts": round(h_pts,1), "p_pts": round(p_pts,1),
            "stat_str": stat_str, "found": entry is not None
        })
    return round(total, 1), results
...
def build_html(cache, games_processed):
    ...
    for i, team in enumerate(teams):
        ...
        for p in sorted(team["players"], key=lambda x: -x["pts"]):
            ...
            pts_str = (f"+{p['pts']}" if p["pts"] > 0 else str(p["pts"])) if p["found"] else "—"
            rows += f"""
            <tr>
              <td><span class="pos-badge">{p["pos"]}</span>{flag} {p["name"]}</td>
              <td class="stat-cell">{p["stat_str"]}</td>
        ...
        <div class="team-card {rank_class}" style="--team-color:{color}" data-owner="{team['owner']}">
          ...
          <div class="score-block">
            <div class="score-pts">{team['score']}</div>
            <div class="score-label">PTS</div>
          </div>
          <div class="stat-pills">
            <div class="stat-pill pos">HIT <span>+{hit_pts:.1f}</span></div>
            <div class="stat-pill pos">PITCH <span>+{pit_pts:.1f}</span></div>
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .roster-table td.stat-cell {{ font-family:'DM Mono',monospace; font-size:.68rem; color:var(--muted); }}
...
if __name__ == "__main__":
    ...
    for owner, score in sorted(teams, key=lambda x: -x[1]):
        print(f"  {owner:12} {score:6.1f} pts")
